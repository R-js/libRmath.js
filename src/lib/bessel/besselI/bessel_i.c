...
/* .Internal(besselI(*)) : */
double bessel_i(double x, double alpha, bool expo)
{
	int nb, ize;
	double na;
	...
	BesselRC rc = I_bessel(x, alpha, nb, ize);
	...
	*/
	...
	if (nb > 0 && x >= 0. && (0. <= nu && nu < 1.) &&
		...
		if (ize == 1 && x > exparg_BESS) {
			...
			return (BesselRC) { ML_POSINF, nb, ncalc };
		}
	...
	/* -------------------------------------------------------------------
	   ...
	   ------------------------------------------------------------------- */
			...
			nend = n - nb;
			bool gotoL230 = false;
			bool gotoL220 = false; //for doc purpose, because L220 follows the for loop immediatly
		...
		//L230:
			...
			return (BesselRC) { bi2[nb - 1], nb, ncalc };
		}
		else { /* small x  < 1e-4 */
		 /* -----------------------------------------------------------
