...
static BesselRC J_bessel(const double x, const double alpha, const int nb);
...
extern double bessel_j(double x, double alpha)
{
	...
	BesselRC rc = J_bessel(x, alpha, nb);
...
static BesselRC J_bessel(const double x, const double alpha, const int nb)
{
	...
	//const int lim = nb;
	double b2[100000] = { 0 };
	int nend, nbmx, i, j, k, l, m, n, nstart;
	...
	if (x < 1 / 10000) {
		...
		printf("x > 25 and nb < int(x+1) :x=%E, nb=%d \t", x, nb);
		/* ------------------------------------------------------------
		...
		for (i = 1; i <= 2; ++i) {
			...
			if (nb == 1) {
				//return (BesselRC) { 0, nb, ncalc };
				return (BesselRC){ b2[nb - 1], nb, ncalc };
			}
...
int main() {
	...
	const double mus[] = { 100 + 0.2, 50, 5, 3, 2, 1. / 2, 1. / 3, 1. / 5 ,1. / 50,1. / 100,1. / 1000,1. / 10000,1.2e-5 };
	// besselJ(0.5, c(100, 50, 5, 3, 2))
	//[1] 6.663900e-219  2.590558e-95  8.053627e-06  2.563730e-03  3.060402e-02
	for (int i = 0; i < 13; i++) {
		double x = 0.5;
		printf("bessel(%f, %f)=\t %.32g\n", x, mus[i], bessel_k(x, mus[i], false));
	}
