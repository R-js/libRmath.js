...
#define M_LN_SQRT_PI  0.57236494292470008
#define M_SQRT_2dPI  0.797884560802865355879892119869

static BesselRC K_bessel(double x, double alpha, int nb, int ize);
...
extern double bessel_k(double x, double alpha, bool expo)
{
	...
	BesselRC rc = K_bessel(x, alpha, nb, ize);
			...
			if (ex <= 0 || (ize == 1 && ex > xmax_BESS_K)) {
				if (ex <= 0) {
					...
					for (i = 0; i < nb; i++)
						bk[i] = 0.;
				ncalc = nb;
				return (BesselRC) { bk[nb - 1], nb, ncalc };
			}
			k = 0;
			if (nu < sqxmin_BESS_K) {
				nu = 0.;
