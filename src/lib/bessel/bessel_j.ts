...
function J_bessel(input: JBesselProperties): void {
  ...
  if (input.nb > 0 && input.x >= 0 && 0 <= nu && nu < 1) {
    ...
    if (input.x > xlrg_BESS_IJ) {
      ML_ERROR(ME.ME_RANGE, 'J_bessel', printer_bessel_j);
    ...
    for (i = 1; i <= input.nb; ++i) input.b[i - 1] = 0;
    ...
    if (input.x < rtnsig_BESS) {
      ...
      halfx = input.x > enmten_BESS ? 0.5 * input.x : 0;
      aa = nu !== 0 ? pow(halfx, nu) / (nu * Rf_gamma_cody(nu)) : 1;
      bb = input.x + 1 > 1 ? -halfx * halfx : 0;
      input.b[0] = aa + aa * bb / alpem;
      if (input.x !== 0 && input.b[0] === 0) input.ncalc = 0;

      if (input.nb !== 1) {
        if (input.x <= 0) {
          for (n = 2; n <= input.nb; ++n) input.b[n - 1] = 0;
        } else {
          /* ----------------------------------------------
                       Calculate higher order functions.
                       ---------------------------------------------- */
          if (bb === 0) tover = (enmten_BESS + enmten_BESS) / input.x;
          ...
          for (n = 2; n <= input.nb; ++n) {
            ...
            if (input.b[n - 1] === 0 && input.ncalc > n) input.ncalc = n - 1;
          }
        }
      }
      ...
      /* ------------------------------------------------
               ...
               ------------------------------------------------ */
      t = trunc(input.x / (twopi1 + twopi2) + 0.5);
      z = input.x - t * twopi1 - t * twopi2 - (nu + 0.5) / pi2;
      vsin = sin(z);
      vcos = cos(z);
      gnu = twonu;
      for (i = 1; i <= 2; ++i) {
        ...
        capp += 1;
        capq = (capq + 1) * (gnu * gnu - 1) * (0.125 / input.x);
      ...
      if (input.nb > 2)
        for (gnu = twonu + 2, j = 3; j <= input.nb; j++, gnu += 2)
          input.b[j - 1] =
            gnu * input.b[j - 1 - 1] / input.x - input.b[j - 2 - 1];
    } else {
      ...
      if (!gotoL240 && !gotoL250) {
        for (n = n - 1; n >= 2; n--) {
          en -= 2;
          input.b[n - 1] =
            en * input.b[n + 1 - 1] / input.x - input.b[n + 2 - 1];
          m = m ? 0 : 2; /* m = 2 - m failed on gcc4-20041019 */
          if (m !== 0) {
            em -= 1;
            alp2em = em + em + nu;
        ...
        /* ---------------------------------------
                   ...
                   -----------------------------------------*/
        input.b[1 - 1] =
          2 * (nu + 1) * input.b[2 - 1] / input.x - input.b[3 - 1];
      }
      if (!gotoL250) {
        //L240:
        em -= 1;
      ...
      /* ---------------------------------------------------
               ...
               ---------------------------------------------------*/
      /*	    if (nu + 1. != 1.) poor test */
      if (fabs(nu) > 1e-15) sum *= Rf_gamma_cody(nu) * pow(0.5 * input.x, -nu);

      aa = enmten_BESS;
      if (sum > 1) aa *= sum;
      for (n = 1; n <= input.nb; ++n) {
        if (fabs(input.b[n]) < aa) input.b[n - 1] = 0;
        else input.b[n - 1] /= sum;
      }
    }
  } else {
