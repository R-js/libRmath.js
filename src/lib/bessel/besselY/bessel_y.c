...
static BesselRC Y_bessel(const double x, const double alpha, const int nb);
...
extern double bessel_y(double x, double alpha)
{
	...
	const nb = 1 + (int)na;/* nb-1 <= alpha < nb */
	alpha -= (double)(nb - 1);


	BesselRC rc = Y_bessel(x, alpha, nb);
