...
export function Y_bessel(x: number, alpha: number, nb: number): IBesselRC {
    ...
    /*----------------------------------------------------------------------
          ...
          ----------------------------------------------------------------------*/
    const ch = [
        -6.7735241822398840964e-24, -6.1455180116049879894e-23, 2.9017595056104745456e-21, 1.3639417919073099464e-19,
        2.3826220476859635824e-18, -9.0642907957550702534e-18, -1.4943667065169001769e-15, -3.3919078305362211264e-14,
        -1.7023776642512729175e-13, 9.1609750938768647911e-12, 2.4230957900482704055e-10, 1.7451364971382984243e-9,
        -3.3126119768180852711e-8, -8.6592079961391259661e-7, -4.9717367041957398581e-6, 7.6309597585908126618e-5,
        0.0012719271366545622927, 0.0017063050710955562222, -0.07685284084478667369, -0.28387654227602353814,
    ...
    if (!(nb > 0 && 0 <= nu && nu < 1)) {
        debug('violaton: abort nb=%d, nu=%d', nb, nu);
        by[0] = 0;
        ncalc = min(nb, 0) - 1;
        return { x: 0, nb, ncalc };
    }
    ...
    if (ex < DBL_MIN || ex > xlrg_BESS_Y) {
        ...
        debug('range issue: x < DBL_MIN but still bigger then 1e8 x=(%d)', ex);
        ncalc = nb;
