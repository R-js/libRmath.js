...
/* 
Description  https://www.cplusplus.com/reference/cmath/tgamma/
This is a an implementation of lgamma C99 
...
function tgamma(x: number): number {
    ...
    const gamma = 0.57721566490153286060651209; // Euler's gamma constant
    ...
    if (x < 12.0) {
        ...
        let y = x;
        let n = 0;
        const arg_was_less_than_one: boolean = y < 1.0;
...
function lgamma_c99(x: number) {
    ...
    const logGamma = (x - 0.5) * log(x) - x + halfLogTwoPi + series;
    return logGamma;
}
