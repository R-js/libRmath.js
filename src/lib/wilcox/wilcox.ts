...
export function cwilcox(k: number, m: number, n: number): number {
  ...
  if (w[i][j] === undefined) {
    ...
    //#endif
    for (l = 0; l <= c; l++) w[i][j][l] = -1;
  }
  if (w[i][j][k] < 0) {
    if (j === 0)
      /* and hence i == 0 */
      w[i][j][k] = k === 0 ? 1 : 0;
    else w[i][j][k] = cwilcox(k - j, i - 1, j) + cwilcox(k, i, j - 1);
  }
  return w[i][j][k];
}
...
export function dwilcox(
  ...
  m = R_forceint(m);
  n = R_forceint(n);
  if (m <= 0 || n <= 0) return ML_ERR_return_NAN(printer_dwilcox);

  if (fabs(x - R_forceint(x)) > 1e-7) return R_D__0(give_log);
  x = R_forceint(x);
  if (x < 0 || x > m * n) return R_D__0(give_log);
...
export function pwilcox(
  ...
  if (q < 0.0) return R_DT_0(lower_tail, log_p);
  if (q >= m * n) return R_DT_1(lower_tail, log_p);
  ...
  if (q <= m * n / 2) {
    for (i = 0; i <= q; i++) p += cwilcox(i, mm, nn) / c;
  } else {
    q = m * n - q;
    for (i = 0; i < q; i++) p += cwilcox(i, mm, nn) / c;
...
export function qwilcox(
  ...
  if (x === R_DT_1(lower_tail, log_p)) return m * n;
