...
export function R_pow(x: number, y: number): number {
  ...
  if (!R_FINITE(y)) {
    if (x >= 0) {
      if (y > 0)
        /* y == +Inf */
        return x >= 1 ? R_PosInf : 0; /* y == -Inf */
      else return x < 1 ? R_PosInf : 0;
    }
  }
...
export function R_powV(x: number, y: number): number /* = x ^ y */ {
  ...
  if (R_FINITE(x) && R_FINITE(y)) return pow(x, y);
  if (ISNAN(x) || ISNAN(y)) {
    return x + y;
  }
  if (!R_FINITE(x)) {
  ...
  if (!R_FINITE(y)) {
    if (x >= 0) {
      if (y > 0)
        /* y == +Inf */
        return x >= 1 ? ML_POSINF : 0; /* y == -Inf */
      else return x < 1 ? ML_POSINF : 0;
    }
  }
