...
export function pnbinom<T>(xx: T, size: number, prob: number, lowerTail: boolean, logP: boolean): T {
...
export function pnbinom_mu<T>(xx: T, size: number, mu: number, lowerTail: boolean, logP: boolean): T {
    const fx: number[] = Array.isArray(xx) ? xx : ([xx] as any);
    const result = fx.map((x) => {
