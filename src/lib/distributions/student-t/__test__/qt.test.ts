...
const qtLogs = select('qt');
const boundLogs = select('R_Q_P01_boundaries');
const qtDomainWarns = qtLogs("argument out of domain in '%s'");
const qtboundaryWarns = boundLogs("argument out of domain in '%s'");

// qt(p: number, ndf: number, lower_tail: boolean, log_p: boolean): number

describe('qt(x,df, log.p)', function () {
