...
export function qnchisq(p: number, df: number, ncp: number, lower_tail = true, log_p = false): number {
    ...
    if (!lower_tail && ncp >= 80) {
        ...
        p = /* R_DT_qIv(p)*/ log_p ? -Math.expm1(p) : 0.5 - p + 0.5;
        lower_tail = true;
    } else {
        p = pp;
    }
    ...
    if (lower_tail) {
        for (; ux < Number.MAX_VALUE && pnchisq_raw(ux, df, ncp, Eps, rEps, 10000, true, false) < pp; ux *= 2);
        pp = p * (1 - Eps);
        for (
            lx = Math.min(ux0, Number.MAX_VALUE);
            lx > Number.MIN_VALUE && pnchisq_raw(lx, df, ncp, Eps, rEps, 10000, true, false) > pp;
            lx *= 0.5
        );
    } else {
        for (; ux < Number.MAX_VALUE && pnchisq_raw(ux, df, ncp, Eps, rEps, 10000, false, false) > pp; ux *= 2);
        pp = p * (1 - Eps);
        for (
            lx = Math.min(ux0, Number.MAX_VALUE);
            lx > Number.MIN_VALUE && pnchisq_raw(lx, df, ncp, Eps, rEps, 10000, false, false) < pp;
            lx *= 0.5
        );
    }
