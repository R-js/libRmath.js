...
export class Toms708 {
	...
	public static alnrel(a: number) {
		...
		// c
		// 
		const p3 = -.178874546012214e-01;
		const p2 = .405303492862024e+00;
		const p1 = -.129418923021993e+01;
		const q3 = -.845104217945565e-01;
		const q2 = .747811014037616e+00;
		const q1 = -.162752256355323e+01;
		// 
	...
	public static algdiv(a: number, b: number) {
		...
		// c
		// 
		const c5 = -.165322962780713E-02;
		const c4 = .837308034031215E-03;
		const c3 = -.595202931351870E-03;
		const c2 = .793650666825390E-03;
		const c1 = -.277777777760991E-02;
		const c0 = .833333333333333E-01;
		// 
	...
	public static apser(a: number, b: number, x: number, eps: number): number {
		...
		if (((b * eps) > 2.e-2)) {
			...
			c = Math.log(x) + Toms708.psi(b) + g + t;
		}
	...
	public static beta_cdf_values(n_data: NumberW, a: NumberW, b: NumberW, x: NumberW, fx: NumberW): void {
		// C*********************************************************************72
		// C
		...
		if (a < 1) {
			...
			if (a < 2 && b < 2) {
				...
				if (b < 8.0) {
					...
					return w + Math.log(z) + (Toms708.gamln(a) + Toms708.gamln(b) - Toms708.gsumln(a, b));
				} else
					return Toms708.gamln(a) + Toms708.algdiv(a, b);
			} else if (a > 2) {
				if ((b > 1000.0)) {
					...
					return (Math.log(w) - n * Math.log(b)) + (Toms708.gamln(a) + Toms708.algdiv(a, b));
				}
	...
	public static bgrat(a: number,
		...
		if (b * z != 0.0) {
			...
			// C
			r = b * (1.0 + Toms708.gam1(b)) * Math.exp(b * Math.log(z));
			r *= Math.exp(a * lnx) * Math.exp(0.5 * bm1 * lnx);
			u = Toms708.algdiv(b, a) + b * Math.log(nu);
			u = r * Math.exp(-u);
	...
	public static brcmp1(mu: number, a: number, b: number, x: number, y: number) {
		...
		// C  const = 1/sqrt(2*pi)
		// C
		const invSqrtPi = .398942280401433;
		let c, u;
		let apb;
	...
	public static bpser(a: number,
		...
		a0 = Math.min(a, b);
		if (a0 >= 1.0) {
			z = a * Math.log(x) - Toms708.betaln(a, b);
	...
	public static bratio(a: number, b: number,
		x: number,
		y: number,
		w: NumberW,
		w1: NumberW,
		ierr: NumberW, log_p: boolean) {
		...
		if (Math.max(a, b) < 1.0e-3 * eps) {
			if (log_p) {
				if (a < b) {
					w.val = log1p(-a / (a + b)); // notably if a << b
					w1.val = log(a / (a + b));
				}
				else {
	...
	public static erf_values(n_data: NumberW, x: NumberW, fx: NumberW): void {
	...
	static gam1(a: number): number {
		...
		const s2 = .559398236957378e-01;
		const s1 = .273076135303957e+00;
		// 
	...
	public static gamma_inc_values(n_data: NumberW, a: NumberW, x: NumberW, fx: NumberW): void {
	...
	public static gamma_log_values(n_data: NumberW, x: NumberW, fx: NumberW) {
	...
	public static gamln(a: number): number {
		...
		if (a <= 0.8)
			return Toms708.gamln1(a) - Math.log(a);
		...
		if (a < 10.0) {
			...
			return Toms708.gamln1(t - 1.0) + Math.log(w);
		}
	...
	public static gamln1(a: number): number {
		...
		// c
		// 
		const p6 = -.271935708322958e-02;
		const p5 = -.673562214325671e-01;
		const p4 = -.402055799310489e+00;
		const p3 = -.780427615533591e+00;
		const p2 = -.168860593646662e+00;
		const p1 = .844203922187225e+00;
		const p0 = .577215664901533e+00;
		const q6 = .667465618796164e-03;
		const q5 = .325038868253937e-01;
		const q4 = .361951990101499e+00;
		const q3 = .156875193295039e+01;
		const q2 = .312755088914843e+01;
		const q1 = .288743195473681e+01;
		// c
		const r5 = .497958207639485e-03;
		const r4 = .170502484022650e-01;
		const r3 = .156513060486551e+00;
		const r2 = .565221050691933e+00;
		const r1 = .848044614534529e+00;
		const r0 = .422784335098467e+00;
		const s5 = .116165475989616e-03;
		const s4 = .713309612391000e-02;
		const s3 = .101552187439830e+00;
		const s2 = .548042109832463e+00;
		const s1 = .124313399877507e+01;
		// 
	...
	public static gsumln(a: number, b: number): number {
		...
		return Toms708.gamln1(x - 1.0) + Math.log(x * (1.0 + x));
	}
	...
	public static psi(xx: number) {
		...
		const dx0 = 1.461632144968362341262659542325721325e0;
		// C
	...
	public static rexp(x: number) {
		...
		// c
		// 
		const q4 = .595130811860248e-03;
		const q3 = -.119041179760821e-01;
		const q2 = .107141568980644e+00;
		const q1 = -.499999999085958e+00;
		const p2 = .238082361044469e-01;
		const p1 = .914041914819518e-09;
	...
	public static rlog1(x: number) {
		...
		// c
		// 
		const a = 0.566749439387324E-01;
		const b = 0.456512608815524E-01;
		// 
		const p2 = .620886815375787E-02;
		const p1 = -0.224696413112536E+00;
		const p0 = 0.333333333333333E+00;
		const q2 = 0.354508718369557E+00;
		const q1 = -0.127408923933623E+01;
		// 
